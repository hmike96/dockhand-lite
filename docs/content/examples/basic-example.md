...
Setup an empty git repository to store [Build Versions]({{< relref "/usage/build-versions" >}})
...
Create [Global Config]({{< relref "/config/global-config" >}}) file named `global.yaml`:
...
Create a [Repo Config]({{< relref "/config/repo-config" >}}) file named `repo-base.yaml`:
...
Use `dhl promote:list-publish` for the `stage` promotion to determine which artifact versions to promote:
...
Use `dhl promote:list-publish` for the `prod` promotion to determine which artifact versions to promote:
